...
MCP_CAN CAN;
SPI device(PA_7, PA_6, PA_5); // mosi, miso, sck
INT8U Flag_int_CAN;
//Serial pc(USBTX, USBRX); // tx, rx

DigitalOut SPICS(PB_6);

InterruptIn CANISR(PA_10);
...
void MCP_CAN::MCP2515_SELECT()
 { SPICS = 0; }
 
void MCP_CAN::MCP2515_UNSELECT()
 { SPICS = 1; }
 
int MCP_CAN::SPI_transfer(char a)
{
    return (device.write(a));
}
...
INT8U MCP_CAN::mcp2515_init(const INT8U canSpeed)                       /* mcp2515init                  */
{
  ...
  INT8U res;
    ...
    {
#if DEBUG_MODE
      pc.printf("Enter setting mode fall\r\n"); 
#endif
      return res;
    }
#if DEBUG_MODE
    pc.printf("Enter setting mode success \r\n");
#endif
    ...
    {
#if DEBUG_MODE
      pc.printf("set rate fall!!\r\n");
#endif
      return res;
    }
#if DEBUG_MODE
    pc.printf("set rate success!!\r\n");
#endif
    ...
    if ( res == MCP2515_OK ) {
        ...
        {
#if DEBUG_MODE        
          pc.printf("Enter Normal Mode Fall!!\r\n");
#endif           
          return res;
        }
...
#if DEBUG_MODE
          pc.printf("Enter Normal Mode Success!!\r\n");
#endif
...
void MCP_CAN::EnqueueTX(CAN_FRAME& newFrame) {
	char counter;
	char status = mcp2515_readStatus() & 0b01010100; //mask for only the transmit buffer empty bits
